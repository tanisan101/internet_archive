...
class AmazonCreatorsAPI:
    ...
    def serialize(product: Any) -> dict:
        ...
        listing = (
            offers_v2 and getattr(offers_v2, 'listings', None) and offers_v2.listings[0]
        )
        ...
        try:
            ...
            logger.exception(f"AmazonCreatorsAPI.serialize({product})")
            publish_date = None
        ...
        _GENERIC_NODES = {'Books', 'Subjects', 'Departments'}
        # Exclude Amazon-internal node names: UUIDs, anything containing 'ASIN',
        # and nodes that are clearly promotional/campaign labels.
        ...
        categories = uniq(
            ...
            if name
            and name not in _GENERIC_NODES
            and not _UUID_RE.match(name)
            and not _INTERNAL_TERMS.search(name)
        )
